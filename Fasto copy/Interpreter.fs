...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  *)
    | Times(e1, e2, pos) ->
      let v1 = evalExp(e1, vtab, ftab)
      let v2 = evalExp(e2, vtab, ftab)
      match (v1, v2) with
        | (IntVal n1, IntVal n2) -> IntVal (n1 * n2)
        | (IntVal _, _) -> reportWrongType "right operand of *" Int v2 (expPos e2)
        | (_, _) -> reportWrongType "left operand of *" Int v1 (expPos e1)

  | Divide(e1, e2, pos) ->
      let v1 = evalExp(e1, vtab, ftab)
      let v2 = evalExp(e2, vtab, ftab)
      match (v1, v2) with
        | (IntVal n1, IntVal n2) when n2 = 0 ->
  ...
  | And(e1, e2, pos) ->
      let v1 = evalExp(e1, vtab, ftab)
      match v1 with
        | BoolVal false -> BoolVal false   // short-circuit
        | BoolVal true ->
            let v2 = evalExp(e2, vtab, ftab)
            match v2 with
              | BoolVal b2 -> BoolVal b2
              | _ -> reportWrongType "right operand of &&" Bool v2 (expPos e2)
        | _ -> reportWrongType "left operand of &&" Bool v1 (expPos e1)

  | Or(e1, e2, pos) ->
      let v1 = evalExp(e1, vtab, ftab)
      match v1 with
        | BoolVal true -> BoolVal true     // short-circuit
        | BoolVal false ->
            let v2 = evalExp(e2, vtab, ftab)
            match v2 with
              | BoolVal b2 -> BoolVal b2
  ...
  | Not(e1, pos) ->
      ...
      match v1 with
        ...
        | _ -> reportWrongType "operand of not" Bool v1 (expPos e1)

  | Negate(e1, pos) ->
      let v1 = evalExp(e1, vtab, ftab)
      match v1 with
        | IntVal n -> IntVal (-n)
        | _ -> reportWrongType "operand of ~" Int v1 (expPos e1)
